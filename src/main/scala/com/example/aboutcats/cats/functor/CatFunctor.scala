...
object CatFunctor extends App {
  ...
  // Invariant functors and the imap method  example
  ...
  implicit def boxCodec[B](implicit c: Codec[B]): Codec[Box[B]] = new Codec[Box[B]] {
    override def encode(bb: Box[B]): String = c.encode(bb.value)

    override def decode(value: String): Box[B] = Box(c.decode(value))
  ...
  // cat Invariant functors implement a method called imap that is informally equivalent to a combination of map and contramap
